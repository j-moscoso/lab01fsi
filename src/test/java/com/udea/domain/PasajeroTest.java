...
class PasajeroTest {
    ...
    void reservasTest() {
        ...
        assertThat(pasajero.getReservases()).containsOnly(reservaBack);
        assertThat(reservaBack.getPasajero()).isEqualTo(pasajero);

        pasajero.removeReservas(reservaBack);
        assertThat(pasajero.getReservases()).doesNotContain(reservaBack);
        assertThat(reservaBack.getPasajero()).isNull();

        pasajero.reservases(new HashSet<>(Set.of(reservaBack)));
        assertThat(pasajero.getReservases()).containsOnly(reservaBack);
        assertThat(reservaBack.getPasajero()).isEqualTo(pasajero);

        pasajero.setReservases(new HashSet<>());
        assertThat(pasajero.getReservases()).doesNotContain(reservaBack);
        assertThat(reservaBack.getPasajero()).isNull();
    }
}
