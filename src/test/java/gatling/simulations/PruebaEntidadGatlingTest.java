...
public class PruebaEntidadGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all pruebaEntidads").get("/api/prueba-entidads").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new pruebaEntidad")
                        .post("/api/prueba-entidads")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"nombre\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_pruebaEntidad_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created pruebaEntidad").get("${new_pruebaEntidad_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created pruebaEntidad").delete("${new_pruebaEntidad_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PruebaEntidad entity").exec(scn);
